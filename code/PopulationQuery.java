...
public class PopulationQuery
{
    ...
    private int [][] pop3;
    private int [][] pop4;
    private double poptotalfinal = 0;
    ...
    //method that asks user for queries 
    private void setUp(CensusData cd)
    {
        boolean done = false;
         Scanner s = new Scanner(System.in);
        do{
            System.out.print("\n Please enter the west, east, north and south corners of the rectangle you wish to find (q to quit): ");
            String inputW = s.next();
            if (inputW.equals("q"))
                done = true;
            else
            {
                try{
                    west = Integer.parseInt(inputW);
                }
                catch (NumberFormatException ex) 
                {
                    System.out.println("Error: did not input an integer");
                    System.exit(0);
                }
                String inputE = s.next();
                try{
                    east = Integer.parseInt(inputE);
                }
                catch (NumberFormatException ex) 
                {
                    System.out.println("Error: did not input an integer");
                ...
                String inputN = s.next();
                try{
                    north = Integer.parseInt(inputN);
                }
                catch (NumberFormatException ex) 
                {
                    System.out.println("Error: did not input an integer");
                ...
                if(west > x || east > x || south > y || north > y || west < 0 || east < 0 || north < 0 || south < 0)
                {
                    System.out.println("Error: did not input corners that are within bounds of grid");
                    System.exit(0);
                }
                if(west > east || north > south)
                 {
                    System.out.println("Error: did not input corners that represent a real rectangle");
                    System.exit(0);
                }
                if(choice == 1)
                    v1(cd);
    ...
    //linear finding the corners and population of query 
    ...
    //using Fork Join framework to find corners
    ...
    //Using fork join framework to find corners and creating a smart grid that can answer queries in O(1) time 
    ...
    private void v3Process(int [][] pop)
    {
        ...
        System.out.println("\nThe population of the rectangle is " + poprect + " and the total population is " + poptotalfinal);
        System.out.printf("The percentage of the population that the rectangle represents is %5.2f\n", (poprect/poptotalfinal*100) );

    }

    //Using fork join framework to find corners and creating a smart grid using Fork join framework that can answer queries in O(1) time     
    private int[][] v4(CensusData cd)
    {
        poprect = 0;
        poptotal = 0;
        int orig, sub1, sub2, add;
    ...
    //once grid is created, this is all that is needed to answer queries 
    private void v4Process(int [][] pop)
    {
        int orig, sub1, sub2, add;
        sub1 = 0;
        sub2 = 0;
    ...
    //same as ver. 4 but instead of Fork Join Framework, this adds to a grid 
    ...
    //Class that extends Thread to make 4 threads add to the final smart grid in version 5 
    ...
    //Adds to the shared smart grid, uses locks 
    ...
    private int [][] reGrid(int [][] pop)//making the smarter grid
    {
        ...
        class Ver2Corners extends RecursiveTask<Rectangle>//finding corners using fork join framework 
    {
    ...
    //Uses Fork join framework to create parts of the smart grid and then add them together 
    ...
    //making the smart grid (parts of it)
    ...
    //adding the parts of the smart grid together 
